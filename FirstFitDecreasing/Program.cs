...
class BinPacking
{
    static int FirstFitDecreasing(List<Container> containers, int shipWidth, int shipLength)
    {
        containers.Sort((a, b) => b.Width * b.Length - a.Width * a.Length); // Ordena os containers por área em ordem decrescente
    ...
    static void Main()
    {
        ...
        int result = FirstFitDecreasing(containers, shipWidth, shipLength);
        Console.WriteLine($"Número mínimo de navios necessários (First-Fit Decreasing): {result}");
    }
}